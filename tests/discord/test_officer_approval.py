...
async def test_approve_button_permission_denied(mock_interaction, mock_settings):
    ...
    mock_interaction.response.send_message.assert_called_with("❌ Only officers can perform this action.", ephemeral=True)
...
async def test_approve_button_success(mock_interaction, mock_settings, async_session):
    ...
    with patch("views.officer_view.get_engine_and_session_maker") as mock_get_engine:
        ...
        char = await service.create_character(CharacterCreate(
            discord_user_id=123, discord_username="Applicant", name="Thorgar",
            race=CharacterRaceEnum.Orc, class_name=CharacterClassEnum.Warrior,
            backstory="...", trait_1="A", trait_2="B", trait_3="C"
        ))
        ...
        mock_interaction.channel.edit.assert_called_with(locked=True, archived=True, name="[APPROVED] Thorgar")