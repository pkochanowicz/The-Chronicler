...
async def test_registration_finalize_success(mock_interaction, async_session):
    ...
    char_create = CharacterCreate(
        discord_user_id=999888777,  # Unique ID to avoid conflicts with repository tests
    ...
    with patch(
        "flows.registration_flow.get_engine_and_session_maker"
    ) as mock_get_engine:
        ...
        with patch(
            "flows.registration_flow.handle_post_to_recruitment", new_callable=AsyncMock
        ) as mock_post_recruitment:
            # Execute
            await flow.finalize()
            ...
            char = await service.get_character_by_discord_id(999888777)
