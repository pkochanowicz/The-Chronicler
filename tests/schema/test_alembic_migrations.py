...
async def empty_db_connection_string(postgres_container):
    ...
    # Return the URL for the new DB
    new_db_url = root_url.replace("/test", f"/{new_db_name}") # Default container db is usually 'test' or 'postgres'
    # Actually, PostgresContainer default db is 'test'. 
    # Let's just construct it carefully.
...
async def test_alembic_upgrade_head_creates_schema(empty_db_connection_string):
    ...
    async with engine.connect() as conn:
        ...
        assert process_2.returncode == 0, "Idempotency check failed: Running upgrade head twice should be safe."
