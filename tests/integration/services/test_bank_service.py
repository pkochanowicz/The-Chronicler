...
from schemas.db_schemas import Item, GuildBankItem, GuildBankTransaction, BankTransactionTypeEnum, ItemQualityEnum
...
async def test_withdraw_item_success(bank_service, async_session):
    ...
    # 2. Action: Withdraw
    result = await bank_service.withdraw_item(
        user_id=445566,
        item_id=999,
        quantity=5
    )
...
async def test_withdraw_insufficient_funds(bank_service, async_session):
    ...
    with pytest.raises(ValueError, match="Insufficient quantity"):
        await bank_service.withdraw_item(
            user_id=777,
            item_id=555,
            quantity=10
        )
