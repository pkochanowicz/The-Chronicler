...
def test_no_gspread_imported():
    ...
    assert 'gspread' not in dir(character_service), "CharacterService must not import gspread"
    
    # Also check 'services.bank_service'
    from services import bank_service
    assert 'gspread' not in dir(bank_service), "GuildBankService must not import gspread"
