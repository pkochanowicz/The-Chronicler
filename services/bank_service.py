...
from schemas.db_schemas import Item, GuildBankItem, GuildBankTransaction, BankTransactionTypeEnum
...
class GuildBankService:
    ...
    async def deposit_item(self, user_id: int, item_name: str, quantity: int, category: str = "General", notes: str = None) -> bool:
        """
        ...
        if not item:
            ...
            raise ValueError(f"Item '{item_name}' does not exist in the database. Please request an item addition first.")
    ...
    async def withdraw_item(self, user_id: int, item_id: int, quantity: int, notes: str = None) -> bool:
        """
        ...
        if bank_item.count < quantity:
            raise ValueError(f"Insufficient quantity. Requested: {quantity}, Available: {bank_item.count}")
    ...
    async def get_all_items(self):
        ...
        stmt = select(GuildBankItem, Item).join(Item).where(GuildBankItem.count > 0).order_by(GuildBankItem.category, Item.name)
        result = await self.session.execute(stmt)
    ...
    async def get_member_deposits(self, user_id: int, limit: int = 20):
        ...
        stmt = (
            ...
            .where(
                ...
                GuildBankTransaction.transaction_type == BankTransactionTypeEnum.DEPOSIT
            )
