...
class CharacterService:
    ...
    async def get_character_by_id(self, character_id: int) -> Optional[pydantic_models.CharacterInDB]:
        db_character = await self.character_repo.get_character_by_id(character_id)
        if db_character:
            return pydantic_models.CharacterInDB.model_validate(db_character)
        return None

    async def get_character_by_discord_id(self, discord_user_id: int) -> Optional[pydantic_models.CharacterInDB]:
        db_character = await self.character_repo.get_character_by_discord_id(discord_user_id)
        if db_character:
            return pydantic_models.CharacterInDB.model_validate(db_character)
        return None

    async def get_character_by_name(self, name: str) -> Optional[pydantic_models.CharacterInDB]:
    ...
    async def update_character(self, character_id: int, character_data: pydantic_models.CharacterUpdate) -> Optional[pydantic_models.CharacterInDB]:
    ...
    async def bury_character(self, character_id: int, cause_of_death: str, eulogy: Optional[str] = None) -> Optional[pydantic_models.GraveyardInDB]:
...
class GraveyardService:
    ...
    async def get_graveyard_entry_by_id(self, entry_id: int) -> Optional[pydantic_models.GraveyardInDB]:
    ...
    async def get_graveyard_entries_for_character(self, character_id: int) -> List[pydantic_models.GraveyardInDB]:
