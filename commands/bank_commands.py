...
class BankCommands(app_commands.Group):
    ...
    async def deposit(self, interaction: discord.Interaction, item: str, quantity: int, category: str = "General", notes: str = ""):
        ...
        async with session_maker() as session:
            ...
            try:
                success = await service.deposit_item(user_id, item, quantity, category, notes)
                if success:
    ...
    async def withdraw(self, interaction: discord.Interaction, item_id: int):
        ...
        async with session_maker() as session:
            service = GuildBankService(session)
            try:
                success = await service.withdraw_item(user_id, item_id, 1) # Default to 1 for now or add quantity param
                if success:
                     await interaction.followup.send(f"✅ Successfully withdrawn 1x Item `{item_id}`.", ephemeral=True)
            except ValueError as e:
                 await interaction.followup.send(f"❌ Error: {str(e)}", ephemeral=True)
            except Exception as e:
                logger.error(f"Unexpected error in withdraw command: {e}", exc_info=True)
                await interaction.followup.send("❌ An unexpected error occurred.", ephemeral=True)
        ...
        async with session_maker() as session:
            ...
            for bank_item, item_obj in items:
                ...
                if cat not in categories:
                    categories[cat] = []
                categories[cat].append(f"• **{item_obj.name}**: {bank_item.count} (ID: {bank_item.item_id})")
    ...
    async def mydeposits(self, interaction: discord.Interaction):
        ...
        async with session_maker() as session:
            ...
            if not deposits:
                 await interaction.followup.send("You haven't deposited any items recently.", ephemeral=True)
                 return
                 
            embed = discord.Embed(title=f"📦 Recent Deposits by {interaction.user.display_name}", color=discord.Color.blue())
            desc = ""
