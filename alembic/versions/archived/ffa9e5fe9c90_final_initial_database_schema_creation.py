...
def upgrade() -> None:
    ...
    # --- 0. Create ALL Enum Types upfront ---
    # Create all enum types before any table operations to avoid conflicts
    # SQLAlchemy will try to auto-create enums, so we create them manually first
    # Using DO blocks to handle cases where they already exist from previous migrations

    for enum_sql in [
        "CREATE TYPE characterroleenum AS ENUM ('Tank', 'Healer', 'DPS')",
        "CREATE TYPE characterstatusenum AS ENUM ('PENDING', 'REGISTERED', 'REJECTED', 'DECEASED', 'BURIED')",
        "CREATE TYPE characterraceenum AS ENUM ('Human', 'Dwarf', 'NightElf', 'Gnome', 'Orc', 'Troll', 'Tauren', 'Undead', 'Goblin', 'HighElf', 'Other')",
        "CREATE TYPE characterclassenum AS ENUM ('Warrior', 'Paladin', 'Hunter', 'Rogue', 'Priest', 'Shaman', 'Mage', 'Warlock', 'Druid')",
        "CREATE TYPE factionalignmentenum AS ENUM ('Alliance', 'Horde', 'Neutral', 'Hostile')",
        "CREATE TYPE itemqualityenum AS ENUM ('Poor', 'Common', 'Uncommon', 'Rare', 'Epic', 'Legendary')",
        "CREATE TYPE creaturetypeenum AS ENUM ('Beast', 'Demon', 'Dragonkin', 'Elemental', 'Giant', 'Humanoid', 'Mechanical', 'Undead', 'Critter', 'Totem', 'Other')",
        "CREATE TYPE questtypeenum AS ENUM ('Normal', 'Daily', 'Dungeon', 'Raid', 'Profession', 'ClassQuest', 'Legendary')",
        "CREATE TYPE spellschoolenum AS ENUM ('Physical', 'Arcane', 'Fire', 'Frost', 'Nature', 'Shadow', 'Holy')",
        "CREATE TYPE banktransactiontypeenum AS ENUM ('DEPOSIT', 'WITHDRAWAL')",
    ]:
        conn.execute(
            sa.text(
                f"""
    ...
    if "images" not in tables:
        op.create_table(
            ...
            sa.Column(
                "last_accessed_timestamp", sa.DateTime(timezone=True), nullable=True
            ),
            sa.Column(
    ...
    if "item_sets" not in tables:
        op.create_table(
            ...
            sa.Column(
                "items_in_set_ids", postgresql.ARRAY(sa.Integer()), nullable=False
            ),
            sa.Column(
                "created_at",
                sa.DateTime(timezone=True),
    ...
    if "items" not in tables:
        op.create_table(
            ...
            sa.Column(
                "stats_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True
            ),
    ...
    if "talents" not in tables:
        op.create_table(
            ...
            sa.CheckConstraint(
                "talent_column >= 1 AND talent_column <= 4", name="talent_column_range"
            ),
            ...
            sa.ForeignKeyConstraint(
                ["tree_id"], ["talent_trees.id"], ondelete="CASCADE"
            ),
            sa.PrimaryKeyConstraint("id"),
        )
    ...
    if "characters" in tables:
        ...
        op.add_column(
            "characters", sa.Column("discord_user_id", sa.BigInteger(), nullable=False)
        )
        op.add_column(
            "characters",
        ...
        op.add_column(
            "characters", sa.Column("name", sa.String(length=64), nullable=False)
        )
        op.add_column(
            "characters",
            sa.Column(
        ...
        op.add_column(
            "characters", sa.Column("trait_1", sa.String(length=128), nullable=False)
        )
        op.add_column(
            "characters", sa.Column("trait_2", sa.String(length=128), nullable=False)
        )
        op.add_column(
            "characters", sa.Column("trait_3", sa.String(length=128), nullable=False)
        )
        op.add_column(
            "characters",
            sa.Column(
        ...
        op.add_column(
            "characters", sa.Column("is_confirmed", sa.Boolean(), nullable=False)
        )
        op.add_column(
            "characters", sa.Column("request_sdxl", sa.Boolean(), nullable=False)
        )
        op.add_column(
            "characters",
        ...
        op.add_column(
            "characters", sa.Column("forum_post_id", sa.BigInteger(), nullable=True)
        )
        op.add_column(
            "characters",
        ...
        op.add_column(
            "characters", sa.Column("death_cause", sa.String(length=256), nullable=True)
        )
        ...
        op.drop_constraint(
            op.f("characters_discord_id_key"), "characters", type_="unique"
        )
    ...
    if "character_talents" in tables:
        ...
        op.drop_constraint(
            op.f("character_talents_character_id_talent_id_key"),
            "character_talents",
            type_="unique",
        )
        op.create_unique_constraint(
            "uq_character_talent", "character_talents", ["character_id", "talent_id"]
        )
    else:
        op.create_table(
            "character_talents",
            ...
            sa.UniqueConstraint(
                "character_id", "talent_id", name="uq_character_talent"
            ),
