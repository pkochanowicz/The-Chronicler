...
for script in script_tags:
    if script.string:  # Inline script
        ...
        if (
            "talents" in script.string.lower()
            or "specs" in script.string.lower()
            or "tree" in script.string.lower()
        ):
            print("--- Inline Script Content (potential data) ---")
            print(
                script.string[:1000]
...
if not found_data:
    print(
        "No obvious talent data found in script tags. Further manual inspection might be needed."
    )
