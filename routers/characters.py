...
@router.post(
    "/",
    response_model=pydantic_models.CharacterInDB,
...
@router.post(
    "/{character_id}/bury",
    response_model=pydantic_models.GraveyardInDB,
    status_code=status.HTTP_201_CREATED,
)
async def bury_character(
    ...
    if graveyard_entry is None:
        raise HTTPException(
            ...
            detail="Character not found or could not be buried",
        )
    return graveyard_entry


@router.get(
    "/{character_id}/graveyard", response_model=List[pydantic_models.GraveyardInDB]
)
