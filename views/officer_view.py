...
class OfficerControlView(View):
    ...
    async def check_permissions(self, interaction: discord.Interaction) -> bool:
        ...
        if not any(rid in user_roles for rid in allowed_roles):
            await interaction.response.send_message("❌ Only officers can perform this action.", ephemeral=True)
            return False
        return True

    @discord.ui.button(label="Approve", style=discord.ButtonStyle.green, custom_id="officer_approve")
    ...
    async def approve_logic(self, interaction: discord.Interaction):
        ...
        async with session_maker() as session:
            ...
            updated_char = await service.update_character(self.character_id, char_update)
            ...
            vault_channel = self.bot.get_channel(vault_channel_id) or await self.bot.fetch_channel(vault_channel_id)
            
            if not vault_channel:
                await interaction.followup.send("❌ Vault channel not found.", ephemeral=True)
                return

            embeds = interaction.message.embeds if interaction.message else []
            
            thread_name = interaction.channel.name.replace("[PENDING]", "[REGISTERED]") if interaction.channel else f"Character {updated_char.name}"
            ...
            if interaction.channel:
                await interaction.channel.edit(locked=True, archived=True, name=f"[APPROVED] {updated_char.name}")
            
            try:
                user = self.bot.get_user(updated_char.discord_user_id) or await self.bot.fetch_user(updated_char.discord_user_id)
                await user.send(f"🎉 Your character **{updated_char.name}** has been APPROVED! Welcome to Azeroth Bound.\nSheet: {vault_thread_msg.thread.jump_url}")
            except Exception as e:
                logger.warning(f"Failed to DM user: {e}")

    @discord.ui.button(label="Reject", style=discord.ButtonStyle.red, custom_id="officer_reject")
    ...
    async def reject_logic(self, interaction: discord.Interaction, reason: str):
        ...
        async with session_maker() as session:
            ...
            if interaction.channel:
                await interaction.channel.edit(locked=True, archived=True, name=f"[REJECTED] {updated_char.name}")
            
            try:
                user = self.bot.get_user(updated_char.discord_user_id) or await self.bot.fetch_user(updated_char.discord_user_id)
                await user.send(f"❌ Your character **{updated_char.name}** was REJECTED.\nReason: {reason}")
            except Exception as e:
                logger.warning(f"Failed to DM user: {e}")

    @discord.ui.button(label="Request Edit", style=discord.ButtonStyle.secondary, custom_id="officer_request_edit")
    async def request_edit_button(self, interaction: discord.Interaction, button: Button):
        if not await self.check_permissions(interaction): return
    ...
    async def request_edit_logic(self, interaction: discord.Interaction, reason: str):
        ...
        async with session_maker() as session:
            ...
            try:
                user = self.bot.get_user(char.discord_user_id) or await self.bot.fetch_user(char.discord_user_id)
                await user.send(f"📝 Officers have requested edits for **{char.name}**.\nFeedback: {reason}\n\nPlease verify changes in the recruitment thread or re-submit if required.")
                if interaction.channel:
                    await interaction.channel.send(f"📝 Edit requested by {interaction.user.mention}: {reason}")
            except Exception as e:
                logger.warning(f"Failed to DM user: {e}")