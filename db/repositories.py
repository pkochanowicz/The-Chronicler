...
class CharacterRepository:
    ...
    async def create_character(
        self, character: pydantic_models.CharacterCreate
    ) -> db_schemas.Character:
        data = character.model_dump(by_alias=False)
        if "roles" in data and data["roles"]:
            data["roles"] = [
                r.value if hasattr(r, "value") else str(r) for r in data["roles"]
            ]
    ...
    async def get_character_by_discord_id(
        ...
        result = await self.db.execute(
            select(db_schemas.Character).filter(
                db_schemas.Character.discord_user_id == discord_user_id
            )
        )
        return result.scalar_one_or_none()
    ...
    async def update_character(
        self, character_id: int, character: pydantic_models.CharacterUpdate
    ) -> Optional[db_schemas.Character]:
        ...
        if "roles" in update_data and update_data["roles"]:
            update_data["roles"] = [
                r.value if hasattr(r, "value") else str(r) for r in update_data["roles"]
            ]
...
class GraveyardRepository:
    ...
    async def create_graveyard_entry(
        self, entry: pydantic_models.GraveyardCreate
    ) -> db_schemas.Graveyard:
    ...
    async def get_graveyard_entry_by_id(
        self, entry_id: int
    ) -> Optional[db_schemas.Graveyard]:
        result = await self.db.execute(
            select(db_schemas.Graveyard).filter(db_schemas.Graveyard.id == entry_id)
        )
        return result.scalar_one_or_none()

    async def get_graveyard_entries_by_character_id(
        self, character_id: int
    ) -> List[db_schemas.Graveyard]:
        result = await self.db.execute(
            select(db_schemas.Graveyard).filter(
                db_schemas.Graveyard.character_id == character_id
            )
        )
        return result.scalars().all()
